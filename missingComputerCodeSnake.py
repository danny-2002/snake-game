...
class App:
    ...
    def on_init(self):
        ...
        self._running = True
        self._image_surf = pygame.image.load("um.png").convert()
        self._apple_surf = pygame.image.load("fdc.jpg").convert()
    ...
    def on_loop(self):
        ...
        for i in range(0,self.player.length):
            if self.game.isCollision(self.computer.x,self.apple.y,self.computer.x[i], self.computer.y[i],44):
                self.apple.x = randint(2,9) * 44
                self.apple.y = randint(2,9) * 44
